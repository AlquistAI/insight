...
@router.post(
    ...
    response_model=KnowledgeBase,
    status_code=status.HTTP_201_CREATED,
    summary="Upload marker-pdf output MD and PDF files as knowledge base",
)
@error_handler
def upload_marker_kb(
        files: list[UploadFile],
        project_id: str,
        kb_id: str = "",
        source_file: str = "",
    ...
    """
    Upload marker-pdf output MD and PDF files as knowledge base.

    The endpoint expects two files:
      - <name>.pdf: original PDF file used as an input to marker-pdf
      - <name>.md: Markdown file generated by marker-pdf with page markers between pages
    ...
    if not len(files) == 2:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "You must provide exactly two files (MD and PDF)")

    files = {
        ext: file for file in files for ext in ("md", "pdf")
        if file.filename and file.filename.lower().endswith(f".{ext}")
    }
