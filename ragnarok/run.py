...
from ragnarok import COMPONENT_ID, COMPONENT_NAME
from ragnarok.api.router import api_router
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        version=CONFIG.RAGNAROK_VERSION,
        lifespan=lifespan,
...
async def lifespan(app: FastAPI):
    logger.info(
        ...
        COMPONENT_NAME, COMPONENT_ID, datetime.now(tz=tz.UTC), CONFIG.RAGNAROK_LOG_LEVEL,
    )
    yield
...
if __name__ == "__main__":
    with suppress(KeyboardInterrupt):
        uvicorn.run(
            app=fast_app,
            host="0.0.0.0",
            port=CONFIG.RAGNAROK_PORT,
            log_level=CONFIG.RAGNAROK_LOG_LEVEL.lower(),
        )
